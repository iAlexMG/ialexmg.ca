...
const TRADUCTIONS = {
  fr: {
    ...
    "crypto.intro": "Le marché crypto ne dort jamais : le Bitcoin (BTC/USDT) se négocie 24 h sur 24, 7 jours sur 7, et contrairement aux marchés boursiers, les exchanges exposent publiquement des données très détaillées — chaque transaction, chaque mouvement du carnet d'ordres.\n\nL'objectif de ce projet : automatiser le trading. Cela exige d'abord de recevoir et d'afficher les données du marché en temps réel, puis de valider chaque stratégie par un backtesting sur l'historique avant de la laisser tourner. D'où les quatre piliers ci-dessous.",

    // Hub Indices boursiers (intro spécifique)
    "indices.intro": "Les futures d'indices du CME — le NQ (Nasdaq 100) et l'ES (S&P 500) — jouent sur un terrain différent de la crypto : le marché ouvre et ferme à heures fixes, et les données détaillées y sont plus rares et plus coûteuses.\n\nL'objectif reste le même : automatiser le trading. Cela exige d'abord de recevoir et d'afficher les données du marché en temps réel, puis de valider chaque stratégie par un backtesting sur l'historique avant de la laisser tourner. D'où les quatre piliers ci-dessous.",
